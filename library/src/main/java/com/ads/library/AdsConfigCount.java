...
public class AdsConfigCount {
    Activity context;
    Dialog pd;

    public AdsConfigCount(Activity context) {
    ...
    public void AdShowCount(Intent intent, int type, boolean isShow) {
        ...
        if (isOnline()) {
            if (PreferenceManager.AppAdcounter(context) == PreferenceManager.Adcounter(context)) {
                PreferenceManager.AppAdcounter(0, context);
                intentAfterInterstitial(intent, type);
            } else if (PreferenceManager.AppAdcounter(context) == 0) {
                PreferenceManager.AppAdcounter(PreferenceManager.AppAdcounter(context) + 1, context);
                if (isShow) {
                    pd.show();
                }
    ...
    void GoogleInter(final Intent intent, final int type) {
        try {
            ...
            interstitialAd.setAdUnitId(PreferenceManager.Other_3(context));
